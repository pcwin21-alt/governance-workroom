...
## 국무조정실 레퍼런스 언어 반영 — 2026-09-08

- Task contract: 앞선 언어 분석을 공개 소개와 워크룸의 대메뉴·소메뉴·상태·행동 문구에 반영한다.
- Active: Root AGENTS, Harness & Skill Inventory Router, `codex-delivery-loop`, Website Product Planning Harness, Eval. 메뉴·카피만 정비하므로 서버 API, 권한, 기록 데이터, 외부 연동은 바꾸지 않는다.
- Decision: 공개 소개는 `회의 기록 / 정책 제안 / 검토 경과 / 제출본·회신 / 자료실`의 업무 흐름으로 다시 썼다. 공개 내비게이션은 `소개 / 체험 / 기록 예시 / 운영 기준`으로 축소했다. 워크룸의 기본 메뉴는 `홈 / 내 활동 / 회의 / 정책 제안 / 자료실 / 내 계정`을 유지하며, `정책 케이스`, `회의록`, `내 참석`, `대시보드` 같은 병렬 표현은 각각 `정책 제안`, `회의 기록`, `참석 응답`, `워크룸`으로 정리했다.
- Boundary: 실제 대외 공개 문서에만 `보도자료`를 사용한다. 외부 공개 승인, 기관 전달·회신의 API, 자료실의 기존 아카이빙 식별자는 변경하지 않는다.
- Eval plan: JavaScript 구문, 자동 테스트, 공개 데모 빌드와 공개 체험 첫 화면의 메뉴·소개 문구를 확인한다.

---

## 내 활동 실행 허브 고도화 — 2026-09-08

- Task contract: `내 활동`을 별도 개인 워크스페이스가 아닌 공동 회의·정책 제안·자료실에서 발생한 개인의 다음 행동과 기록을 모아 확인하는 화면으로 개선한다.
- Active: Root AGENTS, Harness & Skill Inventory Router, `codex-delivery-loop`, Eval. 외부 서비스 연동·서버 스키마·권한 정책·새 개인 데이터 저장은 제외했다.
...
## 핵심 흐름 외 기능 정리 — 2026-09-08

- Task contract: 워크룸을 `회의 기록 → 정책 제안 → 제출·보관`과 개인의 다음 행동에 집중시키고, 중복 탐색과 실제 도입 전 불필요한 보조 기능을 화면에서 제거한다.
...
## 제안 목록 속성과 공통 추가·관리 메뉴 — 2026-09-08

- Task contract: 정책 제안서 목록에서 제안자·제안 일자·상태를 즉시 판독하게 하고, 모든 내부 화면 최상단 우측에 역할별 추가·관리 메뉴를 둔다.
- Active: Root AGENTS, Harness & Skill Inventory Router, `codex-delivery-loop`, Eval. 외부 데이터·권한 정책·상태 값 자체의 변경은 제외했다.
- Decision: 목록 열은 `상태 / 정책 제안 / 제안자·제안 일자 / 분야·피드백`으로 고정했다. 상태는 텍스트와 점을 함께 쓰며 작성 중·검토 중·보완 중·제출 준비·제출 완료·보관됨에 각기 다른 색을 적용했다. 공통 상단 메뉴는 모든 대시보드에 표시하며, 일반위원에게는 제안서 작성·내 계정 관리, 운영 권한 사용자에게는 제안서 작성·회의 추가·구성원·권한 관리만 제공한다.
- Eval evidence: `node --check public/app.js`, `npm test`(36 passed), `npm run build:public-demo`, `git diff --check`를 통과했다. 공개 체험판 첫 화면에서 모든 대시보드에 공통 `추가·관리` 메뉴가 최상단 우측에 표시되고, 위원장 메뉴가 3개 역할별 항목을 제공하는 것을 확인했다. 제안 목록의 최종 시각 검수는 배포 캐시 갱신 후 수행한다.

---

## 자료실 분리 구조 복구 — 2026-09-08

- Task contract: 자료실만 직전 통합 이전의 분리된 탐색 구조로 되돌린다.
- Decision: `전체 기록 / 파일 관리 / 역대 기록` 소메뉴와 각 화면의 분리 진입을 복구했다. 정책 제안 목록과 공통 추가·관리 메뉴 변경은 유지한다.
- Eval plan: 구문·자동 테스트·공개 데모 빌드와 자료실의 세 소메뉴 렌더를 확인한다.

---

## 대시보드 액션 레일·여백 점검 — 2026-09-08

- Task contract: 전 화면의 공통 `추가·관리`와 화면별 상단 행동이 겹쳐 생기는 밀림·빈 공간을 제거하고, 회의 화면의 생성·출결 기능은 하나의 일관된 위치에서 제공한다.
- Active: Root AGENTS, Harness & Skill Inventory Router, `codex-delivery-loop`, Eval. 외부 서비스 연동·서버 데이터·권한 정책 변경은 제외했다.
- Decision: 회의 화면의 중복 상단 버튼 묶음과 그 빈 툴바, 정책 제안 현황의 중복 작성 버튼을 렌더 단계에서 제거했다. 공통 메뉴에 `회의 참석 통계`를 추가해 회의 추가·통계·정책 제안서 작성·구성원 권한 관리가 한 줄기 동작으로 유지된다. 공통 메뉴 아래 간격도 10px에서 6px로 줄여 내용 카드가 불필요하게 밀리지 않게 했다.
- Eval plan: 데스크톱과 작은 화면에서 회의 카드·달력·메뉴의 순서, 메뉴 키보드 접근, 제안·자료실 화면의 공통 메뉴 중복 여부를 확인한다.

---

## 위원장 관리 진입 분리 — 2026-09-08

- Task contract: 공동 작업에 쓰는 메뉴와 위원장 전용 관리 기능을 분리하고, 위원장이 개인 계정 화면에서도 관리 공간으로 진입할 수 있게 한다.
...
- Decision: 정책제안서 상세, 회의 기록 카드, 선택한 회의의 일정 요약에 현재 버전 `DOCX`와 `PDF로 저장`을 표시한다. 원본 회의·제안은 그대로 두며, PDF는 출력 화면을 연 뒤 브라우저 인쇄에서 저장하는 흐름으로 명시한다. 정적 공개 체험판은 서버형 DOCX 경로를 호출하지 않고, 합성 기록을 Word 호환 파일과 출력 화면으로 생성한다.
- Eval plan: 세 진입점의 버튼 표출, DOCX·출력 URL의 기록 파일 식별자, 작은 화면에서 버튼 줄바꿈을 확인한다.

---

## 넓은 화면 작업 캔버스 — 2026-09-08

- Task contract: 좌측 탐색 영역을 유지하면서도 대시보드 본문이 넓은 화면의 가용 폭을 채운다.
- Decision: 데스크톱의 본문 `max-width` 제한을 해제하고, 28–32px의 일정한 내부 여백만 둔다. 모바일·태블릿의 기존 안전 여백과 줄바꿈 규칙은 유지한다.
- Eval plan: 넓은 데스크톱에서 본문 우측 공백 제거, 목록의 열 폭·가로 스크롤 여부, 900px 이하의 반응형 여백을 확인한다.

---

## 읽기 중심 작업 폭 — 2026-09-08

- Task contract: 넓은 화면의 빈 공간을 없애려다 정보 행이 지나치게 넓어지는 문제를 바로잡는다.
- Decision: 데스크톱 본문은 최대 1320px로 제한하고 본문 영역 안에서 가운데 정렬한다. 좁은 화면에서는 가용 폭을 그대로 사용해 별도의 가로 스크롤을 만들지 않는다.
- Eval plan: 정책 제안 목록·일정 카드·캘린더에서 줄 길이, 카드 폭, 좌우 스크롤을 함께 점검한다.
