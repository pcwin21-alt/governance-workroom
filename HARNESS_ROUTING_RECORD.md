...
- Decision: 회의 화면의 중복 상단 버튼 묶음과 그 빈 툴바를 렌더 단계에서 제거했다. 공통 메뉴에 `회의 참석 통계`를 추가해 회의 추가·통계·구성원 권한 관리가 한 줄기 동작으로 유지된다. 공통 메뉴 아래 간격도 10px에서 6px로 줄여 내용 카드가 불필요하게 밀리지 않게 했다.
